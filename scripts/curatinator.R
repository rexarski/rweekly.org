...
x <- get_rss_posts(f$URL, since_days_ago = 8)
...
create_text <- function(
  ...
  if (feed_type == "new") {
    ...
    if (add_diffify == TRUE) {
      placeholder <- stringr::str_c(
        placeholder,
        " - [diffify](https://diffify.com/R/|package_name|)"
      )
    }
  } else {
...
process_cranberries <- function(
  ...
  cb_url <- glue::glue(
    "https://dirk.eddelbuettel.com/cranberries/cran/{feed_type}/index.rss"
  )
  ftype <- feed_type
  ...
  cb_tidy <- tidyRSS::tidyfeed(cb_url) %>%
    ...
    distinct() %>%
    mutate(
      package_name = purrr::map_chr(item_link, ~ tidy_package_name(.x))
    ) %>%

    # leverage the awesome pkgsearch package to get metadata
    mutate(
      package_meta = purrr::map(
        package_name,
        ~ search_cran_package_insistently(.x)
      ),
    ...
    filter(
      item_pub_date >= as.Date(start_date) & item_pub_date <= as.Date(now())
    ) %>%
    select(one_of(c(
      "package_version",
      "package_name",
    ...
    mutate(
      markdown_string = purrr::pmap_chr(
        select(., -package_date),
        create_text,
        add_update_titles = TRUE
      )
    )
